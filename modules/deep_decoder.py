'''
    This is a modified version of the original code hosted at https://github.com/reinhardh/supplement_deep_decoder
    
    Specifically:
        - 1d and 3d versions of deep decoder are added
...
def conv3d(in_f, out_f, kernel_size, stride=1, pad='zero'):
    padder = None
    to_pad = int((kernel_size - 1) / 2)
    if pad == 'reflection':
...
def conv1d(in_f, out_f, kernel_size, stride=1, pad='zero'):
    padder = None
    to_pad = int((kernel_size - 1) / 2)
    if pad == 'reflection':
...
def decodernw3d(
    ...
    for i in range(len(num_channels_up)-1):
        ...
        if upsample_first:
            model.add(conv3d( num_channels_up[i], num_channels_up[i+1],  filter_size_up[i], 1, pad=pad))
            if upsample_mode!='none' and i != len(num_channels_up)-2:
                model.add(nn.Upsample(scale_factor=2, mode=upsample_mode))
            #model.add(nn.functional.interpolate(size=None,scale_factor=2, mode=upsample_mode))	
        else:
        ...
        if i != len(num_channels_up)-1:	
            if(bn_before_act): 
                model.add(nn.BatchNorm3d( num_channels_up[i+1] ,affine=bn_affine))
            model.add(act_fun)
            if(not bn_before_act): 
                model.add(nn.BatchNorm3d( num_channels_up[i+1], affine=bn_affine))
      
    model.add(conv3d( num_channels_up[-1], num_output_channels, 1, pad=pad))
    if need_sigmoid:
        model.add(nn.Sigmoid())
...
def decodernw1d(
    ...
    for i in range(len(num_channels_up)-1):
        ...
        if upsample_first:
            model.add(conv1d( num_channels_up[i], num_channels_up[i+1],  filter_size_up[i], 1, pad=pad))
            if upsample_mode!='none' and i != len(num_channels_up)-2:
                model.add(nn.Upsample(scale_factor=2, mode=upsample_mode))
            #model.add(nn.functional.interpolate(size=None,scale_factor=2, mode=upsample_mode))	
        else:
        ...
        if i != len(num_channels_up)-1:	
            if(bn_before_act): 
                model.add(nn.BatchNorm1d( num_channels_up[i+1] ,affine=bn_affine))
            model.add(act_fun)
            if(not bn_before_act): 
                model.add(nn.BatchNorm1d( num_channels_up[i+1], affine=bn_affine))
                
    if out_size is not None:
        model.add(nn.Upsample(size=out_size, mode=upsample_mode))
      
    model.add(conv1d( num_channels_up[-1], num_output_channels, 1, pad=pad))
    if need_sigmoid:
        model.add(nn.Sigmoid())
